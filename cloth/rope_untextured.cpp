...
{	
	...
	void rope()
	{
		...
		double hx = width / 2.0 - (s * clw / 2.0) + (s / 2.0);

		for (int i = 0; i < clw; i++)
		{
			for (int j = 0; j < clh; j++)
			{
				points.push_back(new point(hx + (s * i), j * s + hy));
			}
		...
		#define randv ((double(rand()) / double(RAND_MAX)) * 2.0 - 1.0)
		...
		else if (mouse_r)
		{
			for (int i = 0; i < constraints.size(); i++)
			{
				if (constraints[i].active)
				{
					if 
					(
						(mouse_x - constraints[i].p1->px) * (mouse_x - constraints[i].p1->px) +
						(mouse_y - constraints[i].p1->py) * (mouse_y - constraints[i].p1->py) 

						<= 

						5.0 * 5.0 ||

						(mouse_x - constraints[i].p2->px) * (mouse_x - constraints[i].p2->px) +
						(mouse_y - constraints[i].p2->py) * (mouse_y - constraints[i].p2->py) 
