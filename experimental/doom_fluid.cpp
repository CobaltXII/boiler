...
{	
	...
	const int block_xr = 4;
	const int block_yr = 4;
	...
	void steam() override
	{
		...
		title = "Doomed smoothed particle hydrodynamics (using Boiler)";
	...
	// Animate the fire at a certain index.
	...
	void spread_fire(int idx)
	{
		...
		if (fire_map[idx] - (deviation & 1) * 2 >= 0)
		{
			fire_map[(idx - deviation + 1) - fx_res] = fire_map[idx] - (deviation & 1) * 2;
		}
	}
