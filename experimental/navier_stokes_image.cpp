...
struct fluid
{
	...
	inline void add_density(int x, int y, float amount)
	{
		...
		density_r[idx(x, y)] += amount;
		density_g[idx(x, y)] += amount;
		density_b[idx(x, y)] += amount;
	}
...
struct game: boiler
{
	...
	fluid simulation = fluid(256, 256, 0.01f, 0.0f, 1e-9f);
	...
	void steam() override
	{
		...
		// Print the image to the density field.

		for (int x = 1; x < simulation.x_res - 1; x++)
		for (int y = 1; y < simulation.y_res - 1; y++)
		{
			// Find the pixel coordinates.

			int px = float(x - 1) / float(simulation.x_res - 2) * image_x_res;
			int py = float(y - 1) / float(simulation.y_res - 2) * image_y_res;

			// Fetch the pixel colors.

			unsigned char r = mgetr(image[py * image_x_res + px]);
			unsigned char g = mgetg(image[py * image_x_res + px]);
			unsigned char b = mgetb(image[py * image_x_res + px]);

			// Assign the density values to the simulation.

			simulation.density_r[simulation.idx(x, y)] = r;
			simulation.density_g[simulation.idx(x, y)] = g;
			simulation.density_b[simulation.idx(x, y)] = b;
		}
	}
