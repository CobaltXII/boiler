...
{	
	...
	// Julia set constants (provided by Wikepedia).

	// double jc_re = 0.0 - 0.70123;
	// double jc_im = 0.0 + 0.27015;

	// double jc_re = 0.0 - 0.4;
	// double jc_im = 0.0 + 0.6;

	// double jc_re = 0.0 - 0.70176;
	// double jc_im = 0.0 - 0.38420;

	// double jc_re = 0.0 - 0.8350;
	// double jc_im = 0.0 - 0.2321;

	double jc_re = 0.0 - 0.800;
	double jc_im = 0.0 + 0.156;

	// double jc_re = 0.0 - 0.7269;
	// double jc_im = 0.0 + 0.1889;

	// double jc_re = 0.0 + 0.0;
	// double jc_im = 0.0 - 0.8;
	...
	void steam() override
	{
		...
		title = "The Julia fractal (using Boiler)";
		...
		// Allocate a buffer for the Julia.

		julia_buf = (Uint32*)malloc(width * height * sizeof(Uint32));
	}

	// Render the Julia to a seperate buffer.

	void julia(bool smooth = false)
	{
		for (int y = 0; y < height; y++)
		{
			...
			for (int x = 0; x < width; x++)
			{
				...
				for (n = 0.0; n < max_iter; n++)
				{
					...
					z_im = 2.0 * z_re * z_im + jc_im;

					z_re = z_re2 - z_im2 + jc_re;
				}
	...
	void keydown(SDL_Event e) override
	{
		...
		{
			savebmp("julia.bmp", pixels, width, height);
		}
	...
	void draw() override
	{
		...
		blitrgb(julia_buf, 0, 0, width, height, 0, 0, width, height);
		...
		else if (ml_released)
		{
			...
			// Render the Julia using the new coordinates.
